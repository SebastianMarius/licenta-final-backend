    ...
    async scrape(city: string, forma?: string, minRoms?: number) {
        ...
        try {
            ...
            for (let pageToScrape = 1; pageToScrape <= MAX_PAGES; pageToScrape++) {
                ...
                // const waitForListingChrome = async () => {
                //     try {
                //         await page.waitForSelector(".article-content", {
                //             timeout: pageToScrape === 1 ? 12000 : 5000,
                //         });
                //         return true;
                //     } catch {
                //         return false;
                ...
                // let ready = await waitForListingChrome();
                // if (!ready && pageToScrape === 1) {
                //     await new Promise((r) => setTimeout(r, 2500));
                //     await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
                //     ready = await waitForListingChrome();
                // }
                // if (!ready) break;
